...
class TestADConfig:
    ...
    def test_base_dn_derivation(self) -> None:
        config = ADConfig(server="dc.local", domain="corp.internal.com")
        assert config.effective_base_dn == "DC=corp,DC=internal,DC=com"

    def test_base_dn_explicit(self) -> None:
        config = ADConfig(server="dc.local", domain="local", base_dn="CN=Users,DC=local")
...
class TestADUserProperties:
    def test_enabled_user(self) -> None:
        u = ADUser(
            dn="CN=Test", sam_account_name="test", user_account_control=512,
        )  # NORMAL_ACCOUNT
    ...
    def test_disabled_user(self) -> None:
        u = ADUser(
            dn="CN=Test", sam_account_name="test", user_account_control=514,
        )  # NORMAL_ACCOUNT | ACCOUNTDISABLE
        assert u.is_enabled is False

    def test_admin_user(self) -> None:
        u = ADUser(dn="CN=Admin", sam_account_name="admin", user_account_control=512, admin_count=1)
        assert u.is_admin is True

    def test_kerberoastable_user(self) -> None:
        u = ADUser(
            dn="CN=Svc",
    ...
    def test_asrep_roastable_user(self) -> None:
        u = ADUser(
            dn="CN=User", sam_account_name="user", user_account_control=0x400000,
        )  # DONT_REQ_PREAUTH
        assert u.is_asrep_roastable is True

    def test_delegation_user(self) -> None:
        u = ADUser(
            dn="CN=Del", sam_account_name="del", user_account_control=0x80000,
...
class TestADGroupProperties:
    def test_global_security(self) -> None:
        g = ADGroup(
            dn="CN=Test", sam_account_name="TestGroup", group_type=-2147483646,
        )  # Global Security
        assert g.scope == "global"
        assert g.is_security_group is True

    def test_universal_security(self) -> None:
        g = ADGroup(
            dn="CN=Test", sam_account_name="TestGroup", group_type=-2147483640,
        )  # Universal Security
        assert g.scope == "universal"

    def test_domain_local(self) -> None:
        g = ADGroup(
            dn="CN=Test", sam_account_name="TestGroup", group_type=-2147483644,
        )  # Domain Local Security
        assert g.scope == "domain_local"

    def test_distribution_group(self) -> None:
        g = ADGroup(dn="CN=Dist", sam_account_name="DistGroup", group_type=2)  # Global Distribution
...
class TestADComputerProperties:
    def test_domain_controller(self) -> None:
        c = ADComputer(
            dn="CN=DC01", sam_account_name="DC01$", user_account_control=0x2000,
        )  # SERVER_TRUST_ACCOUNT
        assert c.is_domain_controller is True

    def test_workstation(self) -> None:
        c = ADComputer(
            dn="CN=WS01", sam_account_name="WS01$", user_account_control=4096,
        )  # WORKSTATION_TRUST
        assert c.is_domain_controller is False

    def test_disabled_computer(self) -> None:
        c = ADComputer(
            dn="CN=OLD", sam_account_name="OLD$", user_account_control=4098,
        )  # WORKSTATION | ACCOUNTDISABLE
...
class TestEnumeratorParsing:
    ...
    def test_parse_user_basic(self) -> None:
        entry = {
    ...
    def test_parse_user_kerberoastable(self) -> None:
        entry = {
    ...
    def test_parse_computer_dc(self) -> None:
        entry = {
...
class TestDomainMap:
    ...
    def test_disabled_users(self) -> None:
        dm = self._build_domain_map()
...
class TestADTrustGraph:
    ...
    def test_find_shortest_path_to_da_admin(self) -> None:
        graph = self._build_graph()
    ...
    def test_find_kerberoastable_paths(self) -> None:
        graph = self._build_graph()
    ...
    def test_find_asrep_roastable_targets(self) -> None:
        graph = self._build_graph()
        targets = graph.find_asrep_roastable_targets()
        assert "legacy" in targets

    def test_find_unconstrained_delegation_hosts(self) -> None:
        graph = self._build_graph()
    ...
    def test_get_high_value_targets(self) -> None:
        ...
        assert "Domain Admins" in hv_names or any("Domain Admins" in n for n in hv_names)
        assert "Administrator" in hv_names
        assert "dc01.corp.local" in hv_names

    def test_get_user_effective_groups(self) -> None:
        graph = self._build_graph()
        groups = graph.get_user_effective_groups("Administrator")
        assert "Domain Admins" in groups

    def test_export_bloodhound_json(self) -> None:
        graph = self._build_graph()
    ...
    def test_find_most_exposed_users(self) -> None:
        graph = self._build_graph()
    ...
    def test_find_all_paths_to_da(self) -> None:
        graph = self._build_graph()
...
class TestAttackPathAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_fallback_analysis(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fallback_report_not_empty(self) -> None:
        from navmax.ad.attack_paths import AttackPathAnalyzer
    ...
    @pytest.mark.asyncio
    async def test_fallback_has_critical_paths(self) -> None:
        from navmax.ad.attack_paths import AttackPathAnalyzer
    ...
    @pytest.mark.asyncio
    async def test_fallback_has_risks(self) -> None:
        from navmax.ad.attack_paths import AttackPathAnalyzer
    ...
    @pytest.mark.asyncio
    async def test_attack_path_analysis_dataclass(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_quick_analysis(self) -> None:
...
class TestADVulnScanner:
    ...
    @pytest.mark.asyncio
    async def test_scan_all_finds_kerberoasting(self) -> None:
        ...
        kerb_findings = [f for f in report.findings if f.category == "kerberoasting"]
        assert len(kerb_findings) >= 1

    @pytest.mark.asyncio
    async def test_scan_all_finds_asrep_roasting(self) -> None:
        from navmax.ad.vuln_scanner import ADVulnScanner
        ...
        asrep_findings = [f for f in report.findings if f.category == "asrep_roasting"]
        assert len(asrep_findings) >= 1

    @pytest.mark.asyncio
    async def test_scan_all_finds_delegation(self) -> None:
        from navmax.ad.vuln_scanner import ADVulnScanner
        ...
        del_findings = [f for f in report.findings if f.category == "delegation"]
        # ws01 a la délégation non contrainte
        assert len(del_findings) >= 1

    @pytest.mark.asyncio
    async def test_scan_all_finds_privileged(self) -> None:
        from navmax.ad.vuln_scanner import ADVulnScanner
        ...
        priv_findings = [f for f in report.findings if f.category == "privileged_accounts"]
    ...
    @pytest.mark.asyncio
    async def test_report_detailed(self) -> None:
        from navmax.ad.vuln_scanner import ADVulnScanner
    ...
    @pytest.mark.asyncio
    async def test_quick_vuln_scan(self) -> None:
    ...
    def test_vuln_finding_str(self) -> None:
...
class TestPasswordSprayer:
    ...
    def test_spray_config_modes(self) -> None:
        from navmax.ad.password_spray import SprayConfig, SprayMode
        ...
        assert (
            SprayConfig(
                mode=SprayMode.CUSTOM,
                delay_seconds=60.0,
            ).effective_delay
            == 60.0
        )

    def test_spray_result(self) -> None:
    ...
    def test_spray_session_summary(self) -> None:
    ...
    def test_wordlist_deduplication(self) -> None:
    ...
    def test_wordlist_from_file(self, tmp_path) -> None:
        from navmax.ad.password_spray import PasswordSprayer
    ...
    def test_prepare_user_list_filters_disabled(self) -> None:
        ...
        users = [
            ADUser(dn="CN=A", sam_account_name="active", user_account_control=512),
            ADUser(
                dn="CN=B", sam_account_name="disabled", user_account_control=514,
            ),  # ACCOUNTDISABLE
        ]
    ...
    def test_prepare_user_list_respects_target_list(self) -> None:
        from navmax.ad.connector import ADUser
        from navmax.ad.password_spray import PasswordSprayer, SprayConfig
        ...
        users = [
            ADUser(dn="CN=A", sam_account_name="admin", user_account_control=512),
            ADUser(dn="CN=B", sam_account_name="user1", user_account_control=512),
            ADUser(dn="CN=C", sam_account_name="svc", user_account_control=512),
        ]
        result = sprayer._prepare_user_list(users)
        assert len(result) == 2
    ...
    @pytest.mark.asyncio
    async def test_spray_without_connector_raises(self) -> None:
...
class TestSMSScanner:
    ...
    def test_smb_share_defaults(self) -> None:
    ...
    def test_smb_computer_result_defaults(self) -> None:
    ...
    def test_smb_domain_report(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_scan_domain_no_connector(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_scan_computer_unreachable(self) -> None:
        from navmax.ad.smb_scanner import ADSMSScanner
    ...
    def test_probe_smb_unreachable(self) -> None:
        import asyncio
    ...
    def test_smb_report_summary(self) -> None:
        ...
        report = SMBDomainReport(domain="test.local")
        result = SMBComputerResult(hostname="dc01", smbv1_enabled=True, writable_shares=1)
        result.shares = [
...
class TestADCSSCanner:
    ...
    def test_template_info_defaults(self) -> None:
    ...
    def test_ca_info_defaults(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_scan_without_connector(self) -> None:
    ...
    def test_adcs_finding(self) -> None:
    ...
    def test_check_esc1_detection_logic(self) -> None:
    ...
    def test_check_esc6_detection_logic(self) -> None:
        from navmax.ad.adcs_scanner import (
            ADCSReport,
            ADCSSCanner,
            CAInfo,
    ...
    def test_check_esc2_detection_logic(self) -> None:
        from navmax.ad.adcs_scanner import (
            ADCSReport,
            ADCSSCanner,
            TemplateInfo,
...
class TestADConnectorImpacketMethods:
    ...
    @pytest.mark.asyncio
    async def test_kerberoast_not_connected(self, config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_asrep_roast_not_connected(self, config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_pass_the_hash_not_connected(self, config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_ntlm_not_connected(self, config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_ntlm_bad_host(self, config) -> None:
    ...
    def test_asrep_roast_method_signature(self) -> None:
    ...
    def test_pass_the_hash_method_signature(self) -> None:
    ...
    def test_authenticate_ntlm_method_signature(self) -> None:
