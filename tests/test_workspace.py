...
from navmax.db.models import Base, Target
...
class TestWorkspaceManager:
    ...
    async def test_get(self, session) -> None:
        mgr = WorkspaceManager(session)
    ...
    async def test_update(self, session) -> None:
        mgr = WorkspaceManager(session)
    ...
    async def test_delete_nonexistent(self, session) -> None:
        mgr = WorkspaceManager(session)
        ok = await mgr.delete("nonexistent-id")
        assert ok is False

    async def test_stats(self, session) -> None:
        mgr = WorkspaceManager(session)
    ...
    async def test_add_remove_target(self, session) -> None:
        mgr = WorkspaceManager(session)
    ...
    async def test_get_nonexistent(self, session) -> None:
        mgr = WorkspaceManager(session)
        assert await mgr.get("fake-id") is None

    async def test_update_nonexistent(self, session) -> None:
        mgr = WorkspaceManager(session)
