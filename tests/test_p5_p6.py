...
class TestAIExploitGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_basic(self, ai_engine, sandbox, service) -> None:
    ...
    @pytest.mark.asyncio
    async def test_generate_with_vulnerabilities(self, ai_engine, sandbox) -> None:
        gen = AIExploitGenerator(ai_engine, sandbox)
        svc = ServiceInfo(
            host="10.0.0.1",
            port=80,
    ...
    @pytest.mark.asyncio
    async def test_extract_code_from_markdown(self, ai_engine, sandbox, service) -> None:
        from navmax.ai.providers.base import GenerateResult, ProviderType

        ai_engine.generate = AsyncMock(
            return_value=GenerateResult(
                text="```python\nimport os\nprint('exploit')\n```",
                model="mock",
                provider=ProviderType.OLLAMA,
    ...
    @pytest.mark.asyncio
    async def test_invalid_python_rejected(self, ai_engine, sandbox, service) -> None:
        from navmax.ai.providers.base import GenerateResult, ProviderType
    ...
    @pytest.mark.asyncio
    async def test_sandbox_tested(self, ai_engine, service) -> None:
        sandbox = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_self_heal_on_failure(self, ai_engine, service) -> None:
    ...
    def test_generated_exploit_properties(self) -> None:
...
class TestAutoPivotEngine:
    ...
    def test_identify_high_value_db(self, engine) -> None:
    ...
    def test_identify_no_high_value(self, engine) -> None:
    ...
    def test_pivot_target_sorting(self, engine) -> None:
        targets = [
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_scanner(self, engine, session) -> None:
    ...
    def test_pivot_session_defaults(self) -> None:
...
class TestPolymorphicMutationEngine:
    ...
    def test_different_seeds_produce_different_code(self) -> None:
    ...
    def test_rename_variables_changes_names(self, engine) -> None:
        code = "x = 1\ny = 2\nz = x + y\nprint(z)"
...
class TestMissionOrchestrator:
    ...
    def test_mission_result_success_rate(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_execute_no_planner(self, orchestrator) -> None:
        result = await orchestrator.execute("test")
        assert result.error is not None
        assert "planner" in result.error.lower()

    @pytest.mark.asyncio
    async def test_dry_run(self, orchestrator) -> None:
        planner = AsyncMock()
        from navmax.ai.mission_planner import MissionPhase, MissionPlan

        planner.plan = AsyncMock(
            return_value=MissionPlan(
                objective="test",
                phases=[
    ...
    @pytest.mark.asyncio
    async def test_execute_with_scanner(self, orchestrator) -> None:
        planner = AsyncMock()
        from navmax.ai.mission_planner import MissionPhase, MissionPlan
    ...
    @pytest.mark.asyncio
    async def test_execute_with_exploit_search(self, orchestrator) -> None:
        planner = AsyncMock()
        from navmax.ai.mission_planner import MissionPhase, MissionPlan
        ...
        planner.plan = AsyncMock(
            return_value=MissionPlan(
                ...
                phases=[
                    MissionPhase(
                        "p1", "Exploit Redis", "exploit", parameters={"service_hint": "redis"},
                    ),
                ],
            ),
        )
    ...
    def test_parse_ports(self, orchestrator) -> None:
