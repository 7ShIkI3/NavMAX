...
from navmax.osint.monitor import (
    OSINTMonitor, MonitorSubscription, ChangeAlert,
)
from navmax.osint.graph.semantic_search import (
    SemanticGraphSearch, GraphQueryResult,
)
...
class TestMonitorSubscription:
    ...
    def test_custom_alerts(self):
        sub = MonitorSubscription(id="s2", target="test.com",
                                   alert_on=["ip_change"])
...
class TestOSINTMonitor:
    ...
    def test_diff_subdomain_removed(self, monitor):
    ...
    def test_diff_ip_change(self, monitor):
    ...
    def test_diff_no_change(self, monitor):
    ...
    def test_diff_disabled_alert_type(self, monitor):
        sub = MonitorSubscription(id="s1", target="example.com",
                                   alert_on=["ip_change"])  # no subdomain alerts
        old = {"subdomains": ["www.example.com"]}
        new = {"subdomains": ["www.example.com", "admin.example.com"]}
    ...
    def test_diff_multiple_new_subdomains(self, monitor):
        old = {"subdomains": ["www.example.com"]}
    ...
    @pytest.mark.asyncio
    async def test_check_creates_snapshot(self, monitor):
    ...
    @pytest.mark.asyncio
    async def test_check_detects_change(self, monitor):
        sub = await monitor.subscribe("example.com")
    ...
    @pytest.mark.asyncio
    async def test_notifier_called(self, monitor):
        notified = []
...
class TestSemanticGraphSearch:
    ...
    def ai_engine(self):
        ...
        engine.generate = AsyncMock(return_value=GenerateResult(
            text=__import__('json').dumps({
                "operations": [
                    {"type": "find_entities", "entity_type": "domain",
    ...
    def test_init(self, graph, ai_engine):
        search = SemanticGraphSearch(graph, ai_engine)
        assert search.graph is graph
        assert search.ai is ai_engine

    @pytest.mark.asyncio
    async def test_search_translates_and_executes(self, graph, ai_engine):
        search = SemanticGraphSearch(graph, ai_engine)
    ...
    def test_execute_find_entities(self, graph, ai_engine):
        search = SemanticGraphSearch(graph, ai_engine)
        ops = [{"type": "find_entities", "entity_type": "domain"}]
        entities, relations = search._execute(ops)
        assert len(entities) >= 2  # example.com + admin.example.com
        values = [e["value"] for e in entities]
        assert "example.com" in values

    def test_execute_get_neighbors(self, graph, ai_engine):
        search = SemanticGraphSearch(graph, ai_engine)
