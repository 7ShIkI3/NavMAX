...
from navmax.reporting.generator import (
    AIReportGenerator, AuditReport, ReportFinding,
)
from navmax.integrations import (
    TheHiveConnector, MISPConnector, IntegrationHub, AlertData,
)
...
class TestAuditReport:
    ...
    def test_critical_count(self):
        r = AuditReport(title="Test", findings=[
...
class TestAIReportGenerator:
    ...
    def ai_engine(self):
        ...
        engine.generate = AsyncMock(return_value=GenerateResult(
            text=json.dumps({
                ...
                "findings": [
                    {"title": "Open SSH", "severity": "MEDIUM",
                     "description": "SSH port open", "cve": None,
                     "affected": "10.0.0.1:22", "evidence": "",
            ...
            tokens_used=100, tokens_per_second=50.0, finish_reason="stop",
        ))
        return engine
    ...
    @pytest.mark.asyncio
    async def test_generate_basic(self, ai_engine, mission_result):
    ...
    @pytest.mark.asyncio
    async def test_generate_no_findings(self, mission_result):
    ...
    @pytest.mark.asyncio
    async def test_to_markdown(self, ai_engine, mission_result):
        gen = AIReportGenerator(ai_engine)
        report = await gen.generate(mission_result)
    ...
    @pytest.mark.asyncio
    async def test_to_html(self, ai_engine, mission_result):
        gen = AIReportGenerator(ai_engine)
        report = await gen.generate(mission_result)
