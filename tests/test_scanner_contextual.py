...
class TestServiceIdentification:
    ...
    def test_apache_banner(self) -> None:
        svc, _ver = _identify_service(80, "HTTP/1.1 200 OK\r\nServer: Apache/2.4.49")
        assert svc == "http"

    def test_nginx_banner(self) -> None:
        svc, _ver = _identify_service(80, "HTTP/1.1 404 Not Found\r\nServer: nginx/1.18.0")
        assert svc == "http"

    def test_mysql_banner(self) -> None:
        svc, _ver = _identify_service(3306, "5.5.5-10.5.12-MariaDB\x00...")
        assert svc == "mysql"

    def test_redis_ping(self) -> None:
        svc, _ver = _identify_service(6379, "+PONG\r\n")
        assert svc == "redis"

    def test_redis_error(self) -> None:
        svc, _ver = _identify_service(6379, "-ERR unknown command")
        assert svc == "redis"

    def test_ftp_banner(self) -> None:
        svc, _ver = _identify_service(21, "220 ProFTPD 1.3.5 Server ready")
        assert svc == "ftp"

    def test_guess_by_port(self) -> None:
    ...
    def test_unknown_banner_falls_back_to_port(self) -> None:
        svc, _ver = _identify_service(22, "SOME_UNKNOWN_BANNER")
...
class TestScanResult:
    ...
    def test_enriched(self) -> None:
        r = ScanResult(
            ...
            service="http",
            version="2.4.49",
            probes={"http_tech": {"server": "Apache"}},
        )
...
class TestVulnDatabase:
    ...
    def test_loads_default_signatures(self, db) -> None:
        assert db.count >= 47

    def test_check_apache_vulnerable(self, db) -> None:
        vulns = db.check("apache", "2.4.49")
        assert len(vulns) >= 1
        cves = [v["cve"] for v in vulns]
        assert "CVE-2021-41773" in cves

    def test_check_apache_safe(self, db) -> None:
...
class TestContextualScanEngine:
    ...
    def test_initialization(self, engine) -> None:
        assert engine.vuln_db is None
        assert engine.ai is None

    def test_with_vuln_db(self) -> None:
        vdb = VulnDatabase()
        engine = ContextualScanEngine(vuln_db=vdb)
        assert engine.vuln_db is vdb

    @pytest.mark.asyncio
    async def test_scan_localhost_closed(self, engine) -> None:
