...
from .base import FirewallConfig, FirewallRule, RuleAction, RuleSeverity
...
class CorrelationReport:
    ...
    def summary(self) -> str:
        lines = [
            "=" * 60,
            f"  AD × Firewall Correlation Report",
...
class ADCorrelator:
    ...
    def correlate(
        ...
        report.correlated_rules = sum(
            1 for f in report.findings if f.firewall_rule
        )
    ...
    def _correlate_admin_exposure(
        ...
        if admin_count > 0 and len(risky_rules) > 3:
            report.findings.append(CorrelationFinding(
                ...
                severity=CorrelationSeverity.HIGH if admin_count > 5 else CorrelationSeverity.MEDIUM,
                ad_group="Domain Admins (et assimilés)",
                firewall_rule=f"{len(risky_rules)} règles Any/Any ou à risque",
                business_impact=(
                    "Compromission d'un admin → accès réseau complet "
            ...
            for port in rule.destination_ports:
                ...
                if port_clean in self.SENSITIVE_PORTS:
                    ...
                    report.findings.append(CorrelationFinding(
                        ...
                        business_impact=(
                            f"Bruteforce ou exploitation de {svc_name} possible"
                        ),
                        remediation=(
    ...
    def _correlate_kerberoastable_exposure(
        ...
        kerb_admin = [u for u in kerb_users
                      if hasattr(u, 'is_admin') and u.is_admin]
        ...
        if kerb_admin and len(fw_config.allow_rules) > 0:
            report.findings.append(CorrelationFinding(
                ...
                ad_user=", ".join(
                    getattr(u, 'sam_account_name', str(u))
                    for u in kerb_admin[:5]
                ),
                business_impact=(
            ...
            report.findings.append(CorrelationFinding(
                ...
                business_impact=(
                    "Escalade de privilèges possible via Kerberoasting"
                ),
                remediation=(
    ...
    def _correlate_vpn_privileged(
        ...
        vpn_rules = [r for r in fw_config.allow_rules
                     if any("vpn" in a.lower() or "ssl" in a.lower()
                            for a in r.destination_addresses)]
