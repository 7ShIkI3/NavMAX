...
class ADConfig:
    ...
    username: Optional[str] = None           # Compte (UPN ou SAM)
    password: Optional[str] = None           # Mot de passe
    auth_method: ADAuthMethod = ADAuthMethod.SIMPLE
    use_ssl: bool = True                     # LDAPS (port 636)
    port: Optional[int] = None               # Auto: 636 si SSL, 389 sinon
    base_dn: Optional[str] = None            # Base DN (auto-dérivé du domaine)
    ...
    ca_cert_file: Optional[str] = None       # Fichier CA pour SSL
...
class ADConnector:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    logger.warning("ad_connect_retry",
                                   attempt=attempt + 1,
                                   error=str(e))
                    await asyncio.sleep(2 ** attempt)
                else:
                    raise ADConnectionError(
                        f"Échec connexion AD après {self.config.max_retries + 1} "
                        f"tentatives: {e}"
                    ) from e
    ...
    async def _connect_internal(self) -> None:
        ...
        if use_ssl:
            tls_config = ldap3.Tls(
                validate=ssl.CERT_REQUIRED if self.config.validate_cert
                         else ssl.CERT_NONE,
        ...
        try:
            ...
            while True:
                ...
                success = await asyncio.to_thread(
                    self._connection.search, **search_args
                )

                if not success:
                    error = self._connection.result.get("description", "Unknown")
                    raise ADSearchError(f"Recherche LDAP échouée: {error}")
                ...
                # Vérifier pagination
                cookie = self._connection.result.get("controls", {}).get(
            ...
            raise ADSearchError(
                f"Erreur lors de la recherche LDAP '{search_filter}': {e}"
            ) from e
    ...
    async def search_users(
        ...
        return await self.search(
            ...
            attributes=attributes or [
                ...
                "mail", "title", "department", "company", "physicalDeliveryOfficeName",
                "telephoneNumber", "memberOf", "primaryGroupID",
                "userAccountControl", "badPwdCount", "lastLogon",
                "pwdLastSet", "accountExpires", "adminCount",
                "servicePrincipalName", "logonCount",
                "homeDirectory", "scriptPath", "profilePath",
                "whenCreated", "whenChanged", "description", "cn", "distinguishedName",
            ],
    ...
    async def search_groups(
        ...
        return await self.search(
            ...
            attributes=attributes or [
                ...
                "adminCount", "description", "whenCreated", "whenChanged",
    ...
    async def search_gpos(
        ...
        return await self.search(
            ...
            attributes=attributes or [
                ...
                "flags", "whenCreated", "whenChanged", "cn", "distinguishedName",
            ],
            max_entries=max_entries,
        )
    ...
    async def search_trusts(
        ...
        return await self.search(
            ...
            attributes=attributes or [
                ...
                "trustAttributes", "cn", "distinguishedName",
            ],
        )
    ...
    async def get_domain_info(self) -> dict[str, Any]:
        ...
        return {
            "defaultNamingContext": self._connection.server.info.naming_contexts[0]
                if self._connection.server.info.naming_contexts else "",
            ...
            "configurationNamingContext": self._connection.server.info.configuration_context
                if hasattr(self._connection.server.info, "configuration_context") else "",
    ...
    def _ensure_connected(self) -> None:
        ...
        if not self.is_connected:
            raise ADConnectionError("Non connecté au contrôleur de domaine.")
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ADConnector(server={self.config.server}, "
            f"domain={self.config.domain}, state={state})"
        )
