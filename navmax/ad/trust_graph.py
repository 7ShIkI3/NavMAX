...
class ADTrustGraph:
    ...
    def build(self, domain_map) -> "ADTrustGraph":
        ...
        logger.info("building_trust_graph",
                    domain=self._domain,
    ...
    def _add_group_nodes(self, domain_map) -> None:
        ...
        for group in domain_map.groups:
            ...
            is_high_value = (
                sam_lower in HIGH_VALUE_GROUPS
                or group.admin_count == 1
            )
    ...
    def _add_computer_nodes(self, domain_map) -> None:
        ...
        for computer in domain_map.computers:
            ...
            is_high_value = (
                ...
                or any(kw in hostname.lower() for kw in HIGH_VALUE_COMPUTERS)
            )
            ...
            node = GraphNode(
                ...
                properties={
                    ...
                    "unconstrained_delegation": (
                        (computer.user_account_control & 0x80000) != 0
                    ),
                },
            )
            self._add_node(node)
    ...
    def find_shortest_path_to_da(
        ...
        return self._shortest_path(user_dn, da_group_dn,
                                   description=f"Chemin de {user_sam} vers Domain Admins")
    
    def find_shortest_path_to_target(
        self, source_sam: str, target_sam: str
    ) -> Optional[AttackPath]:
        ...
        return self._shortest_path(source_dn, target_dn,
                                   description=f"Chemin de {source_sam} vers {target_sam}")
    
    def find_all_paths_to_da(
        self, user_sam: str, max_paths: int = 5
    ) -> list[AttackPath]:
        ...
        return self._all_shortest_paths(user_dn, da_group_dn, max_paths,
                                        f"Chemins de {user_sam} vers Domain Admins")
                    ...
                    if nx.has_path(self._graph, node_id, da_group_dn):
                        ...
                        if attack_path:
                            attack_path.risk_score = self._compute_risk(
                                attack_path, is_kerberoastable=True
                            )
    ...
    def find_cross_domain_attack_paths(self) -> list[AttackPath]:
        ...
        for src_domain, dst_domain in trust_edges:
            ...
            for node_id, node in self._node_index.items():
                if node.type != NodeType.USER:
                    continue
                if node.domain != self._node_index.get(src_domain, GraphNode(
                    id="", type=NodeType.DOMAIN, name=""
                )).name:
                    continue
                    ...
                    if target_node.domain == dst_domain and target_node.high_value:
                        try:
                            ...
                            if nx.has_path(self._graph, node_id, target_id):
                                ...
                                if attack_path:
                                    attack_path.risk_score = self._compute_risk(
                                        attack_path, is_cross_domain=True
                                    )
                                    paths.append(attack_path)
        ...
        for node_id, node in self._node_index.items():
            ...
            try:
                ...
                all_paths = list(nx.all_simple_paths(
                    self._graph, node_id, da_group_dn, cutoff=len(path) + 2
                ))
                exposure.append({
                    "user": node.name,
                    "dn": node_id,
    ...
    def _add_edge(
        self, edge_type: EdgeType, source: str, target: str,
    ...
    def _find_user_dn(self, sam: str) -> Optional[str]:
    ...
    def _find_entity_dn(self, name: str) -> Optional[str]:
    ...
    def _shortest_path(
        self, source_dn: str, target_dn: str, description: str = ""
    ) -> Optional[AttackPath]:
        ...
        path_labels = [
            self._node_index.get(n, GraphNode(
                id=n, type=NodeType.USER, name=n
            )).name
            for n in path
        ]
    ...
    def _all_shortest_paths(
        self, source_dn: str, target_dn: str, max_paths: int,
        description: str = ""
    ) -> list[AttackPath]:
        ...
        try:
            all_paths = list(nx.all_shortest_paths(
                self._graph, source_dn, target_dn
            ))[:max_paths]
        except (nx.NodeNotFound, nx.NetworkXNoPath, nx.NetworkXError):
            return []
