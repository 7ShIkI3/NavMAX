...
class ImpactReporter:
    ...
    async def generate(
        ...
        if domain_map:
            kerb_admin = [
                u for u in domain_map.kerberoastable_users
                if u.is_admin
            ]
            if kerb_admin:
                impacts.append(BusinessImpact(
                    ...
                    description=(
                        f"Service accounts with administrative privileges "
                        f"have SPNs configured. An attacker can request a "
                        f"Kerberos ticket, crack the hash offline, and gain "
            ...
            if unconstrained:
                impacts.append(BusinessImpact(
                    ...
                    description=(
                        f"An attacker compromising any of these hosts can "
                        f"impersonate ANY user authenticating to them. "
                        f"Combined with a coercion attack, this leads to "
                        f"immediate domain compromise."
                    ),
            ...
            if len(domain_map.privileged_users) > 10:
                impacts.append(BusinessImpact(
                    ...
                    description=(
                        f"Excessive number of privileged accounts increases "
                        f"the attack surface. Each admin account is a potential "
                        f"entry point for domain compromise."
                    ),
                    level=ImpactLevel.MEDIUM,
                    affected_assets=[
                        u.sam_account_name
                        for u in domain_map.privileged_users[:5]
                    ],
        ...
        if fw_config:
            risky = [
                r for r in fw_config.allow_rules
            ...
            if len(risky) > 5:
                impacts.append(BusinessImpact(
                    ...
                    description=(
                        f"Firewall rules with 'any' source allow traffic "
                        f"from the entire internet, dramatically expanding "
                        f"the attack surface."
                    ),
                    level=ImpactLevel.HIGH,
            ...
            # FW: CVEs critiques
            critical_cves = [
                c for c in fw_config.cve_checks
                if c.severity == "critical" and c.vulnerable
            ]
            if critical_cves:
                impacts.append(BusinessImpact(
        ...
        report.executive_summary = self._build_executive_summary(
            report, domain_map, fw_config, vuln_report
        )
    ...
    def _build_executive_summary(
        self, report, domain_map, fw_config, vuln_report
    ) -> str:
        ...
        if fw_config:
            cvss = sum(
                c.cvss_score for c in fw_config.cve_checks if c.vulnerable
            )
            parts.append(
