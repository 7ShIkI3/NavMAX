...
class ContinuousMonitor:
    ...
    def __init__(self, ad_connector=None, fw_connector=None) -> None:
    ...
    def capture_baseline(
        ...
        # Utilisateurs admin
        if domain_map:
            baseline.admin_users = {u.sam_account_name for u in domain_map.privileged_users}

            # Groupes admin (adminCount=1)
            baseline.admin_groups = {
                g.sam_account_name for g in domain_map.groups if g.admin_count == 1
            }
            ...
            baseline.domain_controllers = {c.dns_hostname for c in domain_map.domain_controllers}
        ...
        logger.info(
            "baseline_captured", domain=baseline.domain, admin_users=len(baseline.admin_users),
        )
    ...
    def check_drift(
        ...
        if current_domain_map:
            current_admins = {u.sam_account_name for u in current_domain_map.privileged_users}
            ...
            current_kerb = {
                u.sam_account_name for u in current_domain_map.kerberoastable_users if u.is_admin
            }
                ...
                if current_fw_config.rule_count != baseline.fw_rule_count:
                    alerts.append(
                        DriftAlert(
                            ...
                            recommendation=("Audit all new/modified firewall rules."),
                        ),
                    )
                else:
                    alerts.append(
        ...
        if alerts:
            logger.info("drift_detected", domain=baseline.domain, alerts=len(alerts))
    ...
    async def monitor_continuous(
        ...
        logger.info(
            "continuous_monitoring_started", domain=baseline.domain, interval=interval_seconds,
        )
        ...
        while True:
            ...
            try:
                ...
                if drift.alerts:
                    logger.warning(
                        "drift_detected", domain=baseline.domain, alerts=len(drift.alerts),
                    )
                    for alert in drift.alerts:
                        if alert.severity in (AlertSeverity.CRITICAL, AlertSeverity.HIGH):
                            logger.error(
                                "drift_critical_alert",
                                title=alert.title,
                                entity=alert.affected_entity,
                            )

            except (RuntimeError, OSError, ValueError) as e:
                logger.exception("monitor_cycle_error", error=str(e))
